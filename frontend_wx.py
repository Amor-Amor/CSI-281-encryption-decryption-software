...
class Encryption_Frame(wx.Frame):
    ...
    def __init__(self, *args, **kw):
        ...
        select_algorithm_label = wx.StaticText(main_panel, label="Select Algorithm: ")
        ...
        self.algorithm_selector = wx.ComboBox(main_panel, value="None", choices=algorithms, style=wx.CB_READONLY)
    ...
    def click_encrypt_button_wx(self, event):
        if self.algorithm_selector.GetStringSelection() == 'Substitution':
            ...
            encoded_text = csi280_codec.PermutationCypher.encode(self.text_entry.GetLineText(0))
            self.output_box.SetValue("")
            self.output_box.write(encoded_text)
        elif self.algorithm_selector.GetStringSelection() == 'Caesar':
            caesar_cypher = csi280_codec.SubstitutionCypher.Caeser(int(self.key_entry.GetLineText(0)))
    ...
    def click_decrypt_button_wx(self, event):
        if self.algorithm_selector.GetStringSelection() == 'Substitution':
            ...
            decoded_text = csi280_codec.PermutationCypher.decode(self.text_entry.GetLineText(0))
            self.output_box.SetValue("")
            self.output_box.write(decoded_text)
        elif self.algorithm_selector.GetStringSelection() == 'Caesar':
            caesar_cypher = csi280_codec.SubstitutionCypher.Caeser(int(self.key_entry.GetLineText(0)))
...
front_end = wx.App()
main_frame = Encryption_Frame(None, title="Encryption/Decryption GUI 2", size=(800,400))
main_frame.Show()
